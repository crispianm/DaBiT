...
class DepthCompletion(BaseNetwork):
    def __init__(self, init_weights=True):
        ...
        self.ss_depth = SoftSplit(
            channel_d, hidden_d, kernel_size, stride, padding, dropout=dropout
        )
        self.add_pos_emb_depth = AddPosEmb(n_vecs, hidden_d)
        self.sc_depth = SoftComp(
            channel_d, hidden_d, output_size, kernel_size, stride, padding
        )
    ...
    def forward(self, depth_frames, masks):
        ...
        masks = (1.0 - masks).view(b * t, 1, h, w)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.ffn = FusionFeedForward(
            hidden, dim_hidden, p=dropout, n_vecs=n_vecs, t2t_params=t2t_params
        )
...
class TransformerBlockWithDepth(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.attention = MultiHeadedAttentionWithDepth(
            d1_model=hidden1, d2_model=hidden2, head=num_head, p=dropout
        )
        self.ffn1 = FusionFeedForward(
            hidden1,
            dim_hidden=dim_hidden1,
            p=dropout,
            n_vecs=n_vecs,
            t2t_params=t2t_params,
        )
        self.ffn2 = FusionFeedForward(
            hidden2,
...
class EnhanceBlock(nn.Module):
    def __init__(
        self, dim, pa_frames=2, deformable_groups=16, max_residue_magnitude=10
    ):
