        ...
        for module_name in ["backward_", "forward_"]:
            ...
            for i, idx in enumerate(frame_idx):
                ...
                if i > 0:
                    ...
                    feat_prop = torch.cat(
                        [feat_prop, feat_n2], dim=1
                    )  # two order feat_prop -1 & -2
...
class EdgeDetection(nn.Module):
    def __init__(self, in_ch=2, out_ch=1, mid_ch=16):
        ...
        self.mid_layer_2 = nn.Sequential(nn.Conv2d(mid_ch, mid_ch, 3, 1, 1))
...
class RecurrentFlowCompleteNet(nn.Module):
    def __init__(self, model_path=None):
        ...
        self.mid_dilation = nn.Sequential(
            nn.Conv3d(
                128, 128, (1, 3, 3), (1, 1, 1), padding=(0, 3, 3), dilation=(1, 3, 3)
            ),  # p = d*(k-1)/2
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv3d(
                128, 128, (1, 3, 3), (1, 1, 1), padding=(0, 2, 2), dilation=(1, 2, 2)
            ),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv3d(
                128, 128, (1, 3, 3), (1, 1, 1), padding=(0, 1, 1), dilation=(1, 1, 1)
            ),
            nn.LeakyReLU(0.2, inplace=True),
        )
    ...
    def forward(self, blurry_frames, masked_flows, binary_masks):
        ...
        feat_e1 = (
            feat_e1.permute(0, 2, 1, 3, 4).contiguous().view(-1, c, h_f, w_f)
        )  # b*t c h w
