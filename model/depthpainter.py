...
class DepthPainter(BaseNetwork):
    ...
    def forward(
        ...
        feats_ = self.feat_extract(output.view(-1, 3, h_in, w_in))
        h, w = feats_.shape[2:]
        feats_ = feats_.view(b, t, -1, h, w)
        feats["spatial"] = [feats_[:, i, :, :, :] for i in range(0, t)]

        # compute optical flow using the low-res inputs
        assert output.size(3) >= 64 and output.size(4) >= 64, (
        ...
        for iter_ in [1, 2]:
            for direction in ["backward", "forward"]:
                ...
                if direction == "backward":
                    flows = completed_flows[1]
                elif completed_flows[0] is not None:
                    flows = completed_flows[0]
                else:
                    flows = completed_flows[1].flip(1)

                feats = self.propagate(feats, flows, module)
        print("feat prop done")
        sr_output = self.upsample(output, feats)
        print("upsample done")
        return sr_output
