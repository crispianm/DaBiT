...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--root_path",
        type=str,
        default="your_dataset_root/youtube-vos/JPEGImages",
    )
    parser.add_argument(
        "-o", "--save_path", type=str, default="your_dataset_root/youtube-vos/Flows_flo"
    )
    ...
    for f in tqdm(file_list):
        ...
        len_m = len(m_list)
        for i in tqdm(range(len_m - 1), leave=False):
            save_flow_f = os.path.join(
                save_path, f, f"{m_list[i][:-4]}_{m_list[i+1][:-4]}_f.flo"
            )
            save_flow_b = os.path.join(
                save_path, f, f"{m_list[i+1][:-4]}_{m_list[i][:-4]}_b.flo"
            )

            if os.path.exists(save_flow_f) and os.path.exists(save_flow_b):
                # print("Flow already exists, skipping.")
                continue
            ...
            img1 = F.interpolate(
                input=img1, size=(h_new, w_new), mode="bilinear", align_corners=False
            )
            img2 = F.interpolate(
                input=img2, size=(h_new, w_new), mode="bilinear", align_corners=False
            )
