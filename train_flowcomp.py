...
def main(config):
    ...
    # Model and Trainer
    model = RecurrentFlowCompleteNet().to(config["device"])
    print(config["net"], "network created")

    trainer = core.flow_comp_trainer.FlowCompTrainer(config, prefetcher, model)
    trainer.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--config", default="configs/train_flowcomp.json", type=str
    )
