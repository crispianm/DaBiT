...
class FlowCompTrainer:
    def __init__(self, config, prefetcher, model):
        self.config = config
        self.epoch = 0
        self.iteration = 0
        ...
        # set summary writer
        self.summary = {}
        self.writer = SummaryWriter(os.path.join(config["out_dir"], "gen"))

    def setup_optimizers(self):
        """Set up optimizers."""
        backbone_params = []
        for name, param in self.flowcomp.named_parameters():
    ...
    def load(self):
        ...
        if latest_epoch is not None:
            ...
            self.flowcomp.load_state_dict(dataG)
    ...
    def save(self, it):
        ...
        # configure path
        gen_path = os.path.join(self.config["out_dir"], f"gen_{it:06d}.pth")
        opt_path = os.path.join(self.config["out_dir"], f"opt_{it:06d}.pth")
        print(f"\nsaving model to {gen_path} ...")

        # save checkpoints
        torch.save(self.flowcomp.state_dict(), gen_path)
        torch.save(
            {
                "epoch": self.epoch,
    ...
    def train(self):
        ...
        logging.basicConfig(
            ...
            filename=f"logs/{self.config['out_dir'].split('/')[-1]}.log",
            filemode="w",
        )
    ...
    def _train_epoch(self, pbar):
        ...
        while train_data is not None:
            ...
            # complete flow
            pred_flows_bi, pred_edges_bi = self.flowcomp.forward_bidirect_flow(
                blurry_frames[:, :l_t],
                gt_flows_bi,
                local_masks,
            )
            ...
            flow_loss, warp_loss = self.flow_loss(
                pred_flows_bi, gt_flows_bi, local_masks, gt_local_frames
            )
            flow_loss = flow_loss * self.config["losses"]["flow_weight"]
            warp_loss = warp_loss * 0.01
            self.add_summary(self.writer, "loss/flow_loss", flow_loss.item())
            self.add_summary(self.writer, "loss/warp_loss", warp_loss.item())

            # compute edge loss
            edge_loss = self.edge_loss(pred_edges_bi, gt_edges_bi, local_masks)
            edge_loss = edge_loss * 1.0
            self.add_summary(self.writer, "loss/edge_loss", edge_loss.item())

            loss = flow_loss + warp_loss + edge_loss
            self.add_summary(self.writer, "loss/z_total_loss", loss.item())
            loss.backward()
            self.optimG.step()
            ...
            if self.iteration % 100 == 0 and self.writer is not None:
                ...
                flow_results = torch.cat(
                    [
                        masked_flows_forward_cpu,
                        pred_flows_forward_cpu[t],
                        gt_flows_forward_cpu[t],
                    ],
                    1,
                )
                self.writer.add_image(
                    "img/flow-f:masked-out-gt", flow_results, self.iteration
                )
                ...
                flow_results = torch.cat(
                    [
                        masked_flows_backward_cpu,
                        pred_flows_backward_cpu[t],
                        gt_flows_backward_cpu[t],
                    ],
                    1,
                )
                self.writer.add_image(
                    "img/flow-b:masked-out-gt", flow_results, self.iteration
                )

                # TODO: show edge
                # forward
                gt_edges_forward_cpu = gt_edges_bi[0][0].cpu()
                masked_edges_forward_cpu = (
                    gt_edges_forward_cpu[t] * (1 - local_masks[0][t].cpu())
                ).to(gt_edges_forward_cpu)
                pred_edges_forward_cpu = pred_edges_bi[0][0].cpu()

                edge_results = torch.cat(
                    [
                        masked_edges_forward_cpu,
                        pred_edges_forward_cpu[t],
                        gt_edges_forward_cpu[t],
                    ],
                    1,
                )
                self.writer.add_image(
                    "img/edge-f:masked-out-gt", edge_results, self.iteration
                )
                # backward
                gt_edges_backward_cpu = gt_edges_bi[1][0].cpu()
                masked_edges_backward_cpu = (
                    gt_edges_backward_cpu[t] * (1 - local_masks[0][t + 1].cpu())
                ).to(gt_edges_backward_cpu)
                pred_edges_backward_cpu = pred_edges_bi[1][0].cpu()

                edge_results = torch.cat(
                    [
                        masked_edges_backward_cpu,
                        pred_edges_backward_cpu[t],
                        gt_edges_backward_cpu[t],
                    ],
                    1,
                )
                self.writer.add_image(
                    "img/edge-b:masked-out-gt", edge_results, self.iteration
                )
