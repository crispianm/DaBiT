...
if __name__ == "__main__":
    ...
    for video_name in tqdm(os.listdir(test_dir)):
        ...
        blurry_frames, depths, blur_maps, fps, video_name = read_from_videos(
            test_dir, video_name
        )
        ...
        for neighbor_ids in get_frames(
            args.neighbor_length, video_length, neighbor_stride
        ):
            ...
            with torch.no_grad():
                ...
                pred_img = (ori_pred_img + 1) / 2
                pred_img = pred_img.view(-1, 3, out_h, out_w).permute(0, 2, 3, 1)
        ...
        if args.save_videos:
            ...
            for i in range(len(blurry_frames[0])):
                img = np.array(((blurry_frames[0][i] + 1) / 2).cpu().permute(1, 2, 0) * 255)
